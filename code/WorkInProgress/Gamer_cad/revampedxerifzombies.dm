			...
			if(can_see(src,target,viewrange))
				if(distance <= 1 && !istype(src.wear_mask, /obj/item/clothing/mask) && ZAttackable == 1)
					if(prob(25))
						...
						var/datum/organ/external/affecting = 0
						if(istype(T))
							for(var/datum/organ/external/organ in T.organs)
								if(istype(organ, /datum/organ/external/head))
									affecting = organ
									break
							if(affecting)
...
/client/proc/zombie_event(playercalled)
	...
	if(playercalled == 1) // Set when an admin uses the spawn verb, triggers the input form.
		...
		SpawnChoice = "Full Zombie Event"
		SpawnNum = rand(5, 30)


	var/mob/living/carbon/human/zombie/Z
	var/i
	for(i=0,i<SpawnNum,i++) // Loops to determine zombie numver

		if(SpawnChoice == "At Spawn Point" || SpawnChoice == "Full Event") // Would be neater out of the loop, needs cleaning up.
			if(usr) message_admins("[key_name_admin(usr)] has spawned a zombie", 1)
			var/list/t = list()
			for(var/obj/effect/landmark/zspawn/O in world)
				t += O
			if(!t.len)
				usr<<"No spawn points exist."
				return
			var/obj/effect/landmark/s = pick(t)
			Z = new/mob/living/carbon/human/zombie(s.loc)

		else if(SpawnChoice == "At Current Location")

			Z = new/mob/living/carbon/human/zombie(mob.loc)
