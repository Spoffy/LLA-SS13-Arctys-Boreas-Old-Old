...
/turf/proc/ReplaceWithSnow()
	if(!icon_old) icon_old = icon_state
	var/old_icon = icon_old
	var/old_dir = dir
	var/turf/surface/snow/S = new /turf/surface/snow( locate(src.x, src.y, src.z) )
	S.dir = old_dir
	S.icon_old = old_icon
	return S
...
/turf/simulated/floor/CanPass(atom/movable/mover, turf/target, height=0, air_group=0)
	if ((istype(mover, /obj/vehicle) && !(src.burnt)))
...
/turf/surface/snow/attackby(obj/item/weapon/C as obj, mob/user as mob)
	if (istype(C, /obj/item/weapon/tile))
		playsound(src.loc, 'Genhit.ogg', 50, 1)
		C:build(src)
		C:amount--
