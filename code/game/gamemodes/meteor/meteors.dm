...
/proc/spawn_meteor()
	...
	var/attempting_to_spawn = 0
	var/turf/pickedstart
	var/turf/pickedgoal

	do

		attempting_to_spawn = 0
		...
		if (!istype(pickedstart, /turf/surface/snow) || locate(/obj/shield in pickedstart)) //FUUUCK, should never happen.
			attempting_to_spawn = 1
	while(attempting_to_spawn)
