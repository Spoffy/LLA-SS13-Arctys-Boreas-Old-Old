/obj/item/device/radio
	...
	var/obj/structure/comm_tower/linked_mast //Reduce processing time, store the mast last connected to.
	...
	talk_into(mob/M as mob, message)
		...
		//Generate messages and send to devices
		...
		for(var/mob/R in receive)
			...
			if(R.say_understands(M))
				...
				if(M.voice_message)
					name = M.voice_name
					content = M.voice_message
				else
					name = M.voice_name
					content = M.say_quote(stars(message))

			if(istype(M,/mob/living/silicon/ai))
				rendered_message = "[part_a]<a href='byond://?src=\ref[src];track2=\ref[R];track=\ref[M]'>[name] ([eqjobname]) </a>[part_b][content][part_c]"
			else
				rendered_message = "[part_a][name][part_b][content][part_c]"

			R.show_message(rendered_message, 2)
...
/*
	...
	proc
		...
		accept_rad(obj/item/device/radio/R as obj, message)
			...
			if(radio_controller.check_radio_range(R,src))
				return 1

		link_to_mast()
			if(linked_mast)
				if(!radio_controller.check_radio_range(src,linked_mast))
					linked_mast = null
			if(!linked_mast)
				for(var/obj/structure/comm_tower/T in radio_controller.relay_masts)
					if(radio_controller.check_radio_range(src,T))
						linked_mast = T
						break
