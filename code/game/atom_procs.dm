...
/atom/proc/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/device/detective_scanner))
		text = text("\red [src] has been scanned by [user] with the [W]")
	else if (!( istype(W, /obj/item/weapon/grab) ) || !(istype(W, /obj/item/weapon/cleaner)))
		text = text("\red <B>[] has been hit by [] with []</B>", src, user, W)
	for(var/mob/O in viewers(null,src))
		O.show_message(text,1)
	return
