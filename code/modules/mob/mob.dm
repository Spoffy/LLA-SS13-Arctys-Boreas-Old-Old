...
/mob/proc/show_message(msg, type, alt, alt_type)
	...
	if (type)
		if ((type & 1 && (src.sdisabilities & 1 || (src.blinded || src.paralysis)))) // Visual
			if (!( alt ))
				return
			else
				msg = alt
				type = alt_type
		if ((type & 2 && (src.sdisabilities & 4 || src.ear_deaf))) // Audio
			if (!( alt ))
				return
			else
				msg = alt
	...
	if (type & 2 && (stat == 1 || sleeping > 0) && stat != 2)
...
/mob/get_access()
	var/obj/holding = equipped()
	if(holding)
		return holding.get_access()
	else
		return list()
...
/mob/proc/can_use_hands()
	if(src.handcuffed)
		return 0
	if(src.buckled && istype(src.buckled, /obj/structure/stool/bed)) // buckling does not restrict hands
		return 0
	return ..()
...
/mob/proc/put_in_other_hand(var/obj/item/I)
	if(!I) return
	I.loc = src
	if (hand)
		r_hand = I
