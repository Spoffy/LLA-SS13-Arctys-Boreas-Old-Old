...
/mob/living/carbon/human/update_clothing()
	...
	if(src.buckled)
		if(istype(src.buckled, /obj/structure/stool/bed))
			src.lying = 1
		else
			src.lying = 0
...
/mob/living/carbon/human/proc/update_name()
	...
	var/obj/item/weapon/card/id/I = src.wear_id
	var/id_name = ""

	if(I && istype(I, /obj/item/device/pda))
		var/obj/item/device/pda/temp = I
		id_name = temp.owner
	else if(I && istype(I, /obj/item/weapon/card/id))
		id_name = I.registered

	//Prevent people using names made entirely of spaces.
	var/spaces = 0
	for(var/c = 1, c <= length(id_name), c++)
		if(copytext(id_name,c,c+1) == " ")
			spaces++
	if(spaces == length(id_name))
		id_name = ""
	...
	if ((src.wear_mask && !(src.wear_mask.see_face)) || (src.head && !(src.head.see_face))) // can't see the face
		...
		if (id_name && id_name != src.real_name)
			src.name = "[src.real_name] (as [id_name])"
		else
...
//Used to retrieve a mob's ID card.
/mob/living/carbon/human/proc/retrieve_id()
	if(istype(wear_id,/obj/item/device/pda))
		return wear_id:stored_id
