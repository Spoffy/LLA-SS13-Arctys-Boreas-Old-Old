...
/proc/toggle_adminmsg()
	...
	set desc = "Show/hide admin messages."
	set category = "Admin"
...
/obj/admins
	...
	New(var/caller)
		..()
		if(!caller)
			message_admins("Admin object spawn error. No caller specified.")
			del src

		if(!istype(caller, /client) || !caller:ckey || !(caller:ckey in admins))
			if(usr) usr << "You have been booted for attempted hacking. This has been logged.";
			message_admins("[(usr)? usr : "A player"] has been booted due to a possible hack to become an admin. Please report this to a coder or head.")
			del caller
			del src
...
// Now isn't that much better? IT IS NOW A PROC, i.e. kinda like a big panel like unstable
	...
	if (href_list["adminplayeropts"])
		...
		if (ismob(M) && M.client)
			...
			foo += text("<A href='?src=\ref[src];mute2=\ref[M.client]'>Mute: [(M.client.muted ? "Muted" : "Voiced")]</A> | ")
	...
	if (href_list["prayeropts"])
		...
		var/dat = "<html><head><title>Prayer options for [D.key]</title></head>"
		var/foo = "\[ "
		if (ismob(D) && D.client)
			foo += text("<A HREF='?src=\ref[src];prom_demot=\ref[D.client]'>Promote/Demote</A> | ")
			if(!istype(D, /mob/new_player))
				if(!istype(D, /mob/living/carbon/monkey))
					foo += text("<A HREF='?src=\ref[src];monkeyone=\ref[D]'>Monkeyize</A> | ")
				else
					foo += text("<B>Monkeyized</B> | ")
				if(istype(D, /mob/living/silicon/ai))
					foo += text("<B>Is an AI</B> | ")
				else if(istype(D, /mob/living/carbon/human))
					foo += text("<A HREF='?src=\ref[src];makeai=\ref[D]'>Make AI</A> | ")
				foo += text("<A HREF='?src=\ref[src];tdome1=\ref[D]'>Thunderdome 1</A> | ")
				foo += text("<A HREF='?src=\ref[src];tdome2=\ref[D]'>Thunderdome 2</A> | ")
				foo += text("<A HREF='?src=\ref[src];sendtoprison=\ref[D]'>Prison</A> | ")

				foo += text("<A HREF='?src=\ref[src];revive=\ref[D]'>Heal/Revive</A> | ")
			else
				foo += text("<B>Hasn't Entered Game</B> | ")
			foo += text("<A HREF='?src=\ref[src];forcespeech=\ref[D]'>Say</A> | ")
			foo += text("<A href='?src=\ref[src];mute2=\ref[D.client]'>Mute: [(D.client.muted ? "Muted" : "Voiced")]</A> | ")
			foo += text("<A href='?src=\ref[src];jobban2=\ref[D]'>Jobban</A> | ")
			foo += text("<A href='?src=\ref[src];boot2=\ref[D]'>Boot</A> | ")
		foo += text("<A href='?src=\ref[src];jumpto=\ref[D]'>Jump to</A> | ")
		foo += text("<A href='?src=\ref[src];newban=\ref[D]'>Ban</A> \]")
		dat += text("<body>[foo]</body></html>")
	...
	if (href_list["prom_demot"])
		if (src.rank >= 3)
			...
			//host
				dat += {"
				<A href='?src=\ref[src];chgadlvl=Coder;client4ad=\ref[C]'>Coder</A><BR>				<A href='?src=\ref[src];chgadlvl=Senior Admin;client4ad=\ref[C]'>Senior Admin</A><BR>
				...
				<A href='?src=\ref[src];chgadlvl=Secondary Admin;client4ad=\ref[C]'>SA</A><BR>
				<A href='?src=\ref[src];chgadlvl=Tertiary Admin;client4ad=\ref[C]'>M</A><BR>
				<A href='?src=\ref[src];chgadlvl=Admin Observer;client4ad=\ref[C]'>Admin Observer</A><BR>
				<A href='?src=\ref[src];chgadlvl=Remove;client4ad=\ref[C]'>Remove Admin</A><BR>"}
			else if(src.level == 5)
			//coder
				dat += {"
				<A href='?src=\ref[src];chgadlvl=Senior Admin;client4ad=\ref[C]'>Senior Admin</A><BR>				<A href='?src=\ref[src];chgadlvl=Primary Administrator;client4ad=\ref[C]'>PA</A><BR>
				<A href='?src=\ref[src];chgadlvl=Administrator;client4ad=\ref[C]'>A</A><BR>
				<A href='?src=\ref[src];chgadlvl=Secondary Admin;client4ad=\ref[C]'>SA</A><BR>
				<A href='?src=\ref[src];chgadlvl=Tertiary Admin;client4ad=\ref[C]'>M</A><BR>
				<A href='?src=\ref[src];chgadlvl=Admin Observer;client4ad=\ref[C]'>Admin Observer</A><BR>
			...
			//PA
				dat += {"
				<A href='?src=\ref[src];chgadlvl=Administrator;client4ad=\ref[C]'>A</A><BR>				<A href='?src=\ref[src];chgadlvl=Secondary Admin;client4ad=\ref[C]'>SA</A><BR>
				<A href='?src=\ref[src];chgadlvl=Tertiary Admin;client4ad=\ref[C]'>M</A><BR>
				<A href='?src=\ref[src];chgadlvl=Admin Observer;client4ad=\ref[C]'>Admin Observer</A><BR>
				<A href='?src=\ref[src];chgadlvl=Remove;client4ad=\ref[C]'>Remove Admin</A><BR>"}
			else
	...
	//change admin level
		...
		if(!usr.ckey in admins)
			message_admins("[usr] is a non-admin and has tried to [C] an admin!")
			return
		if(rank == "Remove")
			C.clear_admin_verbs()
			...
			switch(href_list["secretsfun"])
				...
				if("sec_classic1")
					...
					for(var/obj/grille/O in world)
						del(O)
					for(var/obj/vehicle/pod/O in world)
				...
				*/
				...
				if("activateprison")
					...
					var/area/start_location = locate(/area/shuttle/prison/planet)
					...
					for(var/atom/movable/AM as mob|obj in A)
						...
						AM.Move()
					*/
					message_admins("\blue [key_name_admin(usr)] sent the planetary shuttle to the station.", 1)
				if("deactivateprison")
					var/area/start_location = locate(/area/shuttle/prison/station)
					var/area/end_location = locate(/area/shuttle/prison/planet)
					start_location.move_contents_to(end_location)
					/*
					var/A = locate(/area/shuttle_prison)
					for(var/atom/movable/AM as mob|obj in A)
						...
						AM.Move()
					*/
					message_admins("\blue [key_name_admin(usr)] sent the planetary shuttle to the planet.", 1)
	...
	//don't warp them if they aren't ready or are already there
							continue
						H.paralysis += 5
						var/obj/item/weapon/card/id/id = H.retrieve_id()
						if(id)
			...
			switch(href_list["secretsadmin"])
						...
						switch(ticker.mode.config_tag)
							...
							if("nuclear")
								...
								for(var/datum/mind/N in ticker.mode:syndicates)
									...
									dat += "<td><A href='?src=\ref[usr.client];priv_msg=\ref[M.client]'>PM</A></td></tr>"
							...
							if("revolution")
								...
								for(var/datum/mind/N in ticker.mode:head_revolutionaries)
									...
									dat += "<td><A href='?src=\ref[usr.client];priv_msg=\ref[M]'>PM</A></td></tr>"
								...
								for(var/datum/mind/N in ticker.mode:get_living_heads())
									var/mob/M = N.current
									dat += "<tr><td><a href='?src=\ref[src];adminplayeropts=\ref[M]'>[M.real_name]</a>[M.client ? "" : " <i>(logged out)</i>"][M.stat == 2 ? " <b><font color=red>(DEAD)</font></b>" : ""]</td>"
									dat += "<td><A href='?src=\ref[usr.client];priv_msg=\ref[M]'>PM</A></td>"
				...
				if("manifest")
					...
					for(var/mob/living/carbon/human/H in world)
						if(H.ckey)
							var/obj/item/weapon/card/id/id = H.retrieve_id()
							dat += text("<tr><td>[]</td><td>[]</td></tr>", H.name, (id) ? text("[]", id.assignment) : "Unknown Position")
					dat += "</table>"
...
/obj/admins/proc/player()
	...
	for(var/mob/M in mobs)
		if(M.ckey)
			...
			<td align=center><A href='?src=\ref[usr.client];priv_msg=\ref[M]'>PM</A></td>
...
<A href='?src=\ref[src];secretsfun=toggleprisonstatus'>Toggle Planetary Shuttle Status(Use with S/R)</A><BR>
<A href='?src=\ref[src];secretsfun=activateprison'>Send Planetary Shuttle</A><BR>
<A href='?src=\ref[src];secretsfun=deactivateprison'>Return Planetary Shuttle</A><BR>
