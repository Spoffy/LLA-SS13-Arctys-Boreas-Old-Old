...
/client/proc/update_admins(var/rank)
	...
	if(!src.ckey in admins)
		message_admins("[src.ckey] attempted to become an admin but was not found in the admin file!")
		if(holder)
			del holder
		return
	...
	//Below is for adding specific verbs to a single rank or observe verbs.
...
//Distribute by holder level to make sure every rank gets it. Plus, it saves on putting it for every rank seperately.

	if (src.holder)
		src.holder.owner = src
		if (src.holder.level >= -1) //Admin Observer
			src.verbs += /client/proc/cmd_pm_client
			src.verbs += /client/proc/cmd_admin_say

		if (src.holder.level >= 0) //Tertiary Admin
		...
		if(src.holder.level >= 1) //Secondary Admin
			src.verbs += /client/proc/ban_player
			src.verbs += /client/proc/cmd_admin_check_contents
		...
		if(src.holder.level >= 2) //Administrator
			...
			src.verbs += /client/proc/Repair_Map


		if(src.holder.level >= 3) //Primary Adminstrator
			src.listenpray = 1
			src.deadchat = 1
			src.verbs += /client/proc/tprayers
			src.verbs += /client/proc/deadchat					//toggles deadchat
		...
		if(src.holder.level >= 4) //Senior Admin
			src.verbs += /client/proc/cmd_modify_object_variables
			src.verbs += /proc/possess
			src.verbs += /proc/release
			src.verbs += /client/proc/jumptokey
			...
			src.verbs += /client/proc/Save_Map
			src.verbs += /obj/admins/proc/spawn_atom

		if(src.holder.level >= 5) //Coder
		...
		if(src.holder.level >= 6) //Game Master
			src << "You are a game master!"
...
/client/proc/funbutton()
	set category = "Fun"
	set name = "Boom Boom Boom Shake The Room (See Desc)"
	set desc = "Blows up the station. Try not to push this."
	if(!src.holder)
