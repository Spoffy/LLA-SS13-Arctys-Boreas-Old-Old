...
/client/proc/cmd_admin_mute(client/C)
	set category = null
	set name = "Toggle Mute"
	if(!src.holder)
		src << "Only administrators may use this command."
		return
	if (C.holder && (C.holder.level >= src.holder.level))
		alert("You cannot perform this action. You must be of a higher administrative rank!", null, null, null, null, null)
		return
	C.muted = !C.muted

	log_admin("[key_name(src)] has [(C.muted ? "muted" : "voiced")] [key_name(C)].")
	message_admins("[key_name_admin(src)] has [(C.muted ? "muted" : "voiced")] [key_name_admin(C)].", 1)

	C << "You have been [(C.muted ? "muted" : "voiced")]."
...
/client/proc/cmd_admin_add_random_ai_law()
	...
	for(var/mob/living/silicon/ai/M in world)
		if(M.stat != 2 && M.see_in_dark != 0)
			...
			var/area = pick("THE BRIDGE", "THE ARRIVAL SHUTTLE", "CHEMICAL LAB", "GENETICS", "ATMOSPHERICS", "CENTCOM", "AMERICA", "IRELAND", "CANADA", "ROMANIA", "GERMANY", "CHINA", "PLANET SURFACE", "VENUS", "MERCURY", "JUPITER", "URANUS", "NEPTUNE", "PLUTO")
